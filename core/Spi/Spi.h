...
public:
    ...
    esp_err_t AddSlaveDevice(const uint8_t mode,const int ss,const uint8_t command_lengh,const uint8_t address_lengh,const int clock_speed_devider = SPI_MASTER_FREQ_20M);

    esp_err_t WriteCmd(const uint8_t command);
    esp_err_t WriteMultipleBytes( uint8_t* reg_data_buffer, const int byte_length, const uint8_t command=0);
    esp_err_t WriteMultipleBytes(uint16_t* reg_data_buffer, const int byte_length, const uint8_t command=0);
